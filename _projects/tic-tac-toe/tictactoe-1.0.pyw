...
class tictactoe(Tk):
    ...
    def __init__(self,*args,**kwargs):
        ...
        helpm= Menu()
        helpm.add_command(label='Info', command=lambda: self.show_frame(help_ttt))
    ...
    def see_end_result(self):
        frame = end_page(self.container,self)
        frame.grid(row=0,column=0,rowspan=3,columnspan=3,sticky='nsew')
        frame.tkraise()
...
class buttonxo(Frame):
    ...
    def move(self):
        ...
        if not result:
            ...
            result=wins(board,turn)
        if result:
             self.controller.see_end_result()
